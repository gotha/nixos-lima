...
let
  ...
  csvToList = s: lib.filter (f: lib.isString f && f != "") (lib.split "," s);
  makeAbsolute = p: if (lib.substring 0 1 p) == "/" then p else "${pwd}/${p}";
in { imports = map makeAbsolute (csvToList cfgFilesCsv); }
